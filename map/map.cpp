...
void Event::fromJson(const json& j) {
    ...
    if (j.contains("x")) j.at("x").get_to(x);
    if (j.contains("y")) j.at("y").get_to(y);
    if (j.contains("chapter_id")) j.at("chapter_id").get_to(chapter_id);
}
...
void Event::set_handeler(Task _t) {
    task=_t;
}
void Event::event_handeler() {
    task();
}
